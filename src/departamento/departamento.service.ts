...
export class DepartamentoService {
    ...
    async editOne(id: number, data: EditDepartamentoDto){
        ...
        if((await respuesta).affected == 0) throw new NotFoundException("No existe el registro de Departamento que intenta modificar");
        return respuesta;
    }
    ...
    async deleteOne(id: number){
        ...
        if(!respuesta) throw new NotFoundException("No existe el registro de Departamento que desea eliminar");
    ...
    async createOne(data: CreateDepartamentoDto){
        const existe = await this.departamentoRepository.findOne({departamento: data.departamento});
